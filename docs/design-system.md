...
User-requested readability and contact update, 2026-09-19: preserve the approved composition but use medium body/navigation text, semibold generic headings and technical headers, darker secondary text (`#4E5B68`) and 11px footer small print. This explicitly supersedes the earlier weight/secondary-text-color baseline without changing the font family, heading size scale, spacing system or core navy/white/orange palette. Reuse the shared native-link floating contact controls; the green WhatsApp action is a channel-specific control, not a new site accent. The two catalog-backed factory introductions are authorized additions to the existing About and Manufacturing overview areas. See `docs/2026-09-19-readability-contact-factory-update.md` for placeholder-contact and factual boundaries.

User-requested URL update, 2026-09-18: following the HINGETRA branding update, the user also requested removing the old brand from the address bar. The GitHub repository and Pages prefix are now `hingetra-industrial-hinges`. The existing deployment-derived base path updates every navigation URL, image/script path, canonical, Open Graph URL, structured-data URL, sitemap and feed without changing any approved page or product route suffix. The custom domain remains unconfigured. This supersedes the earlier decision to retain the old preview path.

User-authorized branding update, 2026-09-18: the website now uses **HINGETRA** and the business display name **Hingetra Industrial Hinges**, as explicitly requested after the earlier illustration updates. Reuse the supplied logo's English-only horizontal adaptation in the shared header/footer and its hinge emblem as the browser icon. This supersedes the earlier restriction on changing the displayed brand, but does not establish a new registered legal entity. Preserve the original catalog identity and technical evidence internally; contact details, RFQ delivery, domain, global preview indexing gate, page layouts and all non-logo styling are unchanged. Brand identity is centralized in `src/data/site.ts`; prompt and asset provenance are in `docs/research/2026-09-18-hingetra-branding.md`.
...
User-requested process-scene update, 2026-09-18: the manufacturing, engineering-review and dimensional-inspection illustrations were regenerated with fictional Chinese personnel and ordinary Chinese hardware-workplace settings, using the original pin/bearing product photographs as appearance references. Reuse the `-cn.png` assets declared in `src/data/illustrations.ts` for all page, article and share-image placements. Keep the neutral “Illustrative scene” captions and explicit AI-generated ALT text; no depicted person or equipment is company evidence. The approved factory exterior, application/packaging scenes, layout, technical data and original product photographs/drawings remain unchanged. Prompt/provenance: `docs/research/2026-09-18-chinese-industrial-scenes-prompts.md`.
...
The user approved and locked `/contact` on 2026-09-02. Its exact files, section order and preservation rules are recorded in `docs/contact-page-lock.json`. The route adds only Contact current-page awareness and a real `/contact` destination to the shared navigation; its composition and CSS remain route-scoped. On 2026-09-19, the user explicitly requested shortening the Contact form: show name, email and one requirement message first, with optional files, product and company details in native collapsed sections. Company/country become optional only for Contact, and the existing D1 schema is unchanged. Preserve surrounding sections, responsive styling, catalog-derived selector, contact details and secure delivery boundary; see `docs/contact-rfq-configuration.md`. Unconfigured delivery must continue to state “Request checked, not sent.” until a real server-side system is connected and tested before production.
...
- RFQ delivery and storage were authorized on 2026-09-19. The shared and Contact forms still send nothing by default. A configured Cloudflare endpoint plus Turnstile can submit privately only when server bindings/secrets and activation are complete; receipt requires a saved-inquiry JSON acknowledgment, not merely HTTP success. Preserve the shared RFQ field order and design; Contact follows the explicitly authorized compact form above. See `docs/inquiry-storage-setup.md`.
