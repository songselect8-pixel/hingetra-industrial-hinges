...
2026-09-20 临时测试路由：用户要求将询盘通知收件人改为 `sales@hingetra.com`，以便本人登录验收；网站公开联系邮箱仍为 `cindy@hingetra.com`。下文 Cindy 为正式收件方案，测试完成后需恢复并确认实际收件。此次仅修改服务器固定通知收件人，不允许访客指定收件人。

用户于 2026-09-19 同意接通真实询盘邮件，并提出保存询盘数据。2026-09-20，用户确认 Contact 测试通知已在 `sales@hingetra.com` 收到，上传图片也已保存在私有 R2。随后明确要求邮件直接附上客户文件、R2 继续保留一份，并精简邮件正文；新版附件邮件的实际收件仍待部署后验证。搜索收录开关继续保持 `SEARCH_INDEXING_ENABLED=false`。
...
- 2026-09-20：用户已确认 Contact 通知实际收件和 PNG 的 R2 保存。此确认适用于原通知；新版随信附件、产品表单、大文件及正式 Cindy 收件等验收仍需分别完成。
